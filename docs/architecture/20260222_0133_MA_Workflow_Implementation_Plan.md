...
| 6. Closing (28~36주) | 5% | **5%** | 선행조건, 자금흐름표, Closing 바인더 |
...
- **약점**: 거래 관리 축(NDA, IOI/LOI, SPA, Closing, PMI) 거의 부재
...
| **Phase 3** | 16~24주 | AI 계약 분석, 전자서명 (DocuSign), SPA 버전 관리, Closing 체크리스트 |
...
| 카테고리 | 파일 | 내용 |
|:---|:---|:---|
| **Models** | `contract.py` | Contract 모델 (SPA/SHA/NDA 등 7종, 5단계 상태) |
| | `contract_version.py` | ContractVersion 모델 (버전 관리, 변경사항 추적) |
| | `closing_checklist.py` | ClosingChecklist 모델 (8개 카테고리, 4단계 상태) |
| **Schemas** | `contract.py` | Contract/ContractVersion CRUD 스키마 |
| | `closing.py` | ClosingChecklist CRUD + Progress Summary |
| **Routers** | `contracts.py` | `GET/POST/PATCH/DELETE /contracts` + Versions + Summary |
| | `closing.py` | `GET/POST/PATCH/DELETE /closing-checklist` + Summary |
| **Migration** | `003_phase3_contracts_closing.py` | 3 테이블 + 5 enum 타입 |

**Frontend — Phase 3 추가/수정**

| 카테고리 | 파일 | 내용 |
|:---|:---|:---|
| **Types** | `contract.ts` | Contract, ContractVersion, ContractCreate, ClosingChecklist |
| **Hooks** | `useContracts.ts` | contracts + versions CRUD, closing checklist CRUD |
| **Pages** | `TransactionWorkspacePage.tsx` | 8탭 → 10탭 (Contracts, Closing Checklist) |
...
| 카테고리 | 파일 | 내용 |
|:---|:---|:---|
| **Models** | `pmi_task.py` | PMI Task 모델 (8개 카테고리, 5단계 상태, 3단계 우선순위) |
| | `earnout_milestone.py` | Earnout Milestone 모델 (6종 메트릭, 5단계 상태) |
| **Schemas** | `pmi.py` | PMI Task CRUD + Dashboard Summary |
| | `earnout.py` | Earnout Milestone CRUD + Summary |
| **Routers** | `pmi.py` | `GET/POST/PATCH/DELETE /pmi-tasks` + Dashboard |
| | `earnout.py` | `GET/POST/PATCH/DELETE /earnout-milestones` + Summary |
| **Migration** | `004_phase4_pmi_earnout.py` | 2 테이블 + 5 enum 타입 |

**Frontend — Phase 4 추가/수정**

| 카테고리 | 파일 | 내용 |
|:---|:---|:---|
| **Types** | `pmi.ts` | PMITask, PMICreate, PMIDashboard |
| | `earnout.ts` | EarnoutMilestone, EarnoutCreate, EarnoutSummary |
| **Hooks** | `usePMI.ts` | PMI tasks + dashboard CRUD |
| | `useEarnout.ts` | Earnout milestones CRUD |
| **Pages** | `TransactionWorkspacePage.tsx` | 10탭 → 12탭 (PMI, Earnout) |

### Phase 5A: Notes & Approvals — ✅ 완료

**Backend (deal-mgmt) — Phase 5A 추가 파일**

| 카테고리 | 파일 | 내용 |
|:---|:---|:---|
| **Models** | `note.py` | DealNote 모델 (4종 타입, 스레드/핀/멘션/첨부) |
| | `approval.py` | ApprovalRequest 모델 (4종 타입, 4단계 상태, 복수 승인자) |
| **Schemas** | `note.py` | Note CRUD + 스레드/핀 관리 |
| | `approval.py` | Approval CRUD + 승인/거부 결정 |
| **Routers** | `notes.py` | `GET/POST/PATCH/DELETE /notes` + Pin/Thread |
| | `approvals.py` | `GET/POST /approvals` + `POST decide` |
| **Migration** | `005_phase5a_notes_approvals.py` | 2 테이블 + 3 enum 타입 |

**Frontend — Phase 5A 추가/수정**

| 카테고리 | 파일 | 내용 |
|:---|:---|:---|
| **Types** | `note.ts` | DealNote, NoteCreate, NoteUpdate |
| | `approval.ts` | ApprovalRequest, ApprovalCreate, ApprovalDecision |
| **Hooks** | `useNotes.ts` | Notes CRUD + pin/thread |
| | `useApprovals.ts` | Approvals CRUD + decide |
| **Pages** | `TransactionWorkspacePage.tsx` | 12탭 → 14탭 (Notes, Approvals) |

### Phase 5B: Risk & Compliance Matrix — ✅ 완료

**Backend (deal-mgmt) — Phase 5B 추가 파일**

| 카테고리 | 파일 | 내용 |
|:---|:---|:---|
| **Models** | `risk_item.py` | RiskItem 모델 (9개 카테고리, 4단계 심각도, 5단계 가능성, 6단계 상태) |
| | `compliance_item.py` | ComplianceItem 모델 (10개 카테고리, 7단계 상태, 관할권/규제기관) |
| **Schemas** | `risk.py` | Risk CRUD + Matrix Summary |
| | `compliance.py` | Compliance CRUD + Summary + By-Category |
| **Routers** | `risks.py` | `GET/POST/PATCH/DELETE /risks` + Summary |
| | `compliance.py` | `GET/POST/PATCH/DELETE /compliance` + Summary |
| **Migration** | `006_phase5b_risk_compliance.py` | 2 테이블 + 6 enum 타입 |
| **Workflow** | `workflow_engine.py` 수정 | CLOSING 진입 게이트: 미완화 Critical 리스크 및 NON_COMPLIANT 차단 |

**Frontend — Phase 5B 추가/수정**

| 카테고리 | 파일 | 내용 |
|:---|:---|:---|
| **Types** | `risk.ts` | RiskItem, RiskCreate, RiskUpdate, RiskSummary |
| | `compliance.ts` | ComplianceItem, ComplianceCreate, ComplianceSummary |
| **Hooks** | `useRisks.ts` | Risks CRUD + summary |
| | `useCompliance.ts` | Compliance CRUD + summary |
| **Constants** | `constants.ts` 확장 | Risk/Compliance 상태/카테고리/심각도 옵션, 배지 색상 |
| **Pages** | `TransactionWorkspacePage.tsx` | Risk Matrix 탭, Compliance 탭 추가 |

**검증 결과 (Phase 3~5B 전체)**

- `pytest deal-mgmt/tests/ -v`: **172/172 통과** (16.30s) ✅
- `tsc --noEmit`: 에러 0건 ✅
- `vite build`: 성공 (5.44s, 2962 modules) ✅
- **E2E Docker 테스트**: 18/18 통과 ✅
  - Transaction 생성 → 7단계 Phase 전환
  - Risk Gate: Critical 리스크 미완화 시 CLOSING 차단 → 완화 후 통과
  - Compliance Gate: NON_COMPLIANT 항목 시 CLOSING 차단 → 승인 후 통과
  - Dashboard/Summary API 정상
...
| **Phase 3**: Contracts & Closing | ✅ 완료 | 31 | Contract 버전 관리, Closing Checklist |
| **Phase 4**: PMI & Earnout | ✅ 완료 | 31 | PMI 대시보드, Earnout Milestone 추적 |
| **Phase 5A**: Notes & Approvals | ✅ 완료 | 32 | 딜 노트 (스레드/핀), 다단계 승인 워크플로우 |
| **Phase 5B**: Risk & Compliance | ✅ 완료 | 32 | 리스크 매트릭스, 컴플라이언스 체크리스트, CLOSING 게이트 |

### 파일 통계 (전체)

| 카테고리 | 파일 수 | 비고 |
|:---|:---:|:---|
| 백엔드 모델 | 19 | transaction, engagement, working_group, buyer_candidate, timeline, audit, nda, bid, dd_checklist, contract, contract_version, closing_checklist, pmi_task, earnout_milestone, note, approval, risk_item, compliance_item, base+enums |
| 백엔드 스키마 | 15 | transaction, workflow, engagement, buyer, timeline, dashboard, nda, bid, dd_checklist, contract, closing, pmi, earnout, note, approval, risk, compliance |
| 백엔드 라우터 | 15 | transactions, workflow, engagements, buyers, timeline, dashboard, ndas, bids, dd_checklists, contracts, closing, pmi, earnout, notes, approvals, risks, compliance |
| 백엔드 서비스 | 3 | transaction_service, workflow_engine, audit_service |
| 백엔드 코어 | 4 | config, database, exceptions, security |
| 마이그레이션 | 6 | 001_initial ~ 006_phase5b (19 tables) |
| 테스트 | 13 | 172 test cases + E2E Docker test |
| 프론트엔드 타입 | 14 | transaction ~ compliance |
| 프론트엔드 훅 | 8 | useTransactions, useNdas, useBids, useDDChecklist, useContracts, usePMI, useEarnout, useNotes, useApprovals, useRisks, useCompliance |
| 프론트엔드 페이지 | 3 | TransactionList, CreateTransaction, TransactionWorkspace |
| **총계** | **~100** | 백엔드 75 + 프론트엔드 25 + 인프라 |

### 다음 단계

| 우선순위 | 작업 | 상태 |
|:---:|:---|:---:|
| 1 | PR 생성: `feat/ma-workflow` → `master` | ⬜ |
| 2 | 브라우저 E2E 시각 확인 (전체 16탭 동작) | ⬜ |
| 3 | native `<select>` → styled `<Select>` 컴포넌트 전환 (UX 개선) | ⬜ |
| 4 | 인라인 셀 편집 확장 (amount, title, assignee, due_date) | ⬜ |
| 5 | FDD/IM/KIIS 백엔드 API 연동 (cross-service calls) | ⬜ |
| 6 | 전자서명 (DocuSign) 통합 기획 | ⬜ |
