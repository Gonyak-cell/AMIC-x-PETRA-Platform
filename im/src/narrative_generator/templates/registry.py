...
class TemplateRegistry:
    ...
    Examples:
        >>> registry = TemplateRegistry("im/src/narrative_generator/templates")
        >>> registry.has("executive_summary")
        True
        >>> tpl = registry.get("executive_summary", industry="tech")
    """
    ...
    def get(
        ...
        *,
        ...
        Args:
            section_id: 섹션 식별자.
            industry: 산업 식별자 (빈 문자열이면 기본 템플릿).

        Returns:
            SectionTemplate 또는 템플릿 미존재 시 None.
        """
        self._ensure_loaded()
    ...
    def _merge_templates(
        ...
        오버라이드에 정의된 필드만 덮어쓰고, 나머지는 기본 유지.
        slots는 키 단위로 병합 (오버라이드 우선).
        """
