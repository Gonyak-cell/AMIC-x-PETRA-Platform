...
class SlotDefinition:
    ...
    Attributes:
        ...
        source: L2 슬롯의 IMDocumentData 필드 경로 (dotted notation).
        slot_type: L3 슬롯의 출력 형태.
        hint: L3 슬롯의 LLM 가이드 힌트.
        max_tokens: L3 슬롯의 최대 토큰 수.
        data_keys: L3 슬롯이 참조할 IMDocumentData 필드 경로 리스트.
        default: 데이터 없을 때 기본값.
        format_spec: L2 슬롯의 포맷 지정 (예: "currency", "percent", "date").
    """
...
class SectionTemplate:
    ...
    Attributes:
        section_id: 섹션 식별자 (executive_summary, contact 등).
        version: 템플릿 버전.
...
class TemplateLoader:
    ...
    def load_base(self) -> BaseBlocks:
        ...
        if not path.exists():
            logger.warning("_base.yaml 미발견: %s", path)
            return BaseBlocks()
    ...
    def load_section(self, section_id: str) -> SectionTemplate | None:
        ...
        Args:
            section_id: 섹션 식별자 (파일명과 동일).

        Returns:
            SectionTemplate 또는 파일 미존재 시 None.
        """
    ...
    def load_all_sections(self) -> dict[str, SectionTemplate]:
        ...
        Returns:
            {section_id: SectionTemplate} 딕셔너리.
        """
    ...
    def load_industry_override(
        ...
        Args:
            section_id: 섹션 식별자.
            industry: 산업 식별자 (tech, healthcare 등).

        Returns:
            오버라이드 SectionTemplate 또는 None.
        """
    ...
    def _read_yaml(self, path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error("YAML 로드 실패: %s — %s", path, exc)
            return {}

    def _parse_template(self, raw: dict[str, Any]) -> SectionTemplate:
        """YAML 딕셔너리를 SectionTemplate으로 파싱한다."""
