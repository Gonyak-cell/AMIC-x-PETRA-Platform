...
class PatternAggregator:
    ...
    async def get_common_issues(
        ...
        im_ralph_sessions.progress JSONB에서 gate_results → issues를 추출하여
        빈도순으로 정렬한다.
        """
        from src.api.db.models.im_ralph_session import IMRalphSession

        q = (
            select(IMRalphSession.progress)
            .where(
                IMRalphSession.doc_type == doc_type,
                IMRalphSession.status == "COMPLETED",
                IMRalphSession.progress.isnot(None),
            )
            .order_by(IMRalphSession.created_at.desc())
            .limit(20)  # 최근 20개 세션만
        )
    ...
    async def get_best_practices(
        ...
        q = (
            ...
            .where(
                IMRalphSession.doc_type == doc_type,
                IMRalphSession.status == "COMPLETED",
                IMRalphSession.final_score >= min_score,
                IMRalphSession.progress.isnot(None),
            )
            .order_by(IMRalphSession.final_score.desc())
            .limit(10)
        )
        result = await self._db.execute(q)
        rows = result.scalars().all()
