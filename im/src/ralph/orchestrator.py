...
Phase 2: 섹션별 생성-평가 (섹션당 최대 3회)
Phase 3: 통합 검증 (전체 문서 조합)
"""
...
from src.ralph.convergence import ConvergenceChecker, ConvergenceConfig
from src.ralph.gates.base import GateResult, QualityGate
from src.ralph.progress_tracker import ProgressTracker
...
class DocumentGenerator(Protocol):
    ...
    IM PPTX 파이프라인 래퍼가 이 인터페이스를 구현한다.
    """
...
class RalphLoopOrchestrator:
    ...
    사용법::
        ...
        orchestrator = RalphLoopOrchestrator(
            ...
            gates=[IMPPTXProgrammaticGate(), IMVisionGate(...), IMLLMDesignJudge(...)],
            prd=load_prd("im_full"),
        )
    ...
    async def run(self, source_data: dict[str, Any]) -> LoopResult:
        ...
        try:
            ...
            logger.exception("[IM Ralph Loop %s] 실행 실패", self._session_id[:8])
            return LoopResult(
                session_id=self._session_id,
                status=LoopStatus.FAILED,
                total_iterations=self._total_iterations,
    ...
    async def _phase2_iterate(
        ...
        for section in outline:
            ...
            if self._convergence.check_total_budget(self._tracker):
                logger.warning("[IM Ralph Loop] 비용 한도 초과, 남은 섹션 건너뜀")
                break
