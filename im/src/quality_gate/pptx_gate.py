...
import 경로를 IM 모듈용으로 조정하고 동일한 5+1차원 검증을 수행한다.
"""
...
class PPTXProgrammaticGate(QualityGate):
    ...
    async def evaluate(
        ...
        if template_spec is not None:
            slot_score, slot_issues, slot_crits = self._check_slot_compliance(
                prs, template_spec
            )
            ...
            scaled: list[DimensionScore] = [
                DimensionScore(d.name, d.label, d.score, d.weight * 0.85, d.feedback)
                for d in dimensions
            ]
            scaled.append(
                DimensionScore("slot_compliance", "슬롯 적합성", slot_score, 0.15)
            )
            dimensions = scaled
    ...
    def _check_slot_compliance(
        ...
        for slide_idx, slide in enumerate(slides, 1):
            ...
            if slide_spec.required_shapes:
                ...
                missing = [
                    rn for rn in slide_spec.required_shapes if rn not in shape_names
                ]
                if missing:
            ...
            for slot in slide_spec.slots:
                ...
                if slot.content_type == "TEXT":
                    self._check_text_slot(
                        slide, slide_idx, slot, issues, critical_flags
                    )
    ...
    def _check_text_slot(
        ...
        if slot.max_chars is not None and len(text) > slot.max_chars:
            msg = (
                f"슬라이드 {slide_idx} '{slot.name}': "
                f"글자 수 초과 ({len(text)}/{slot.max_chars})"
            )
            if is_fail_policy:
        ...
        if slot.max_lines is not None:
            ...
            if line_count > slot.max_lines:
                msg = (
                    f"슬라이드 {slide_idx} '{slot.name}': "
                    f"줄 수 초과 ({line_count}/{slot.max_lines})"
                )
                if is_fail_policy:
                    critical_flags.append(f"CRITICAL: {msg}")
                else:
            ...
            for run in para.runs:
                if run.font.size is not None and run.font.size < min_emu:
                    ...
                    issues.append(
                        f"슬라이드 {slide_idx} '{slot.name}': "
                        f"폰트 크기 미달 "
                        f"({actual_pt:.1f}pt < {slot.min_font_pt}pt)"
                    )
                    return  # 슬롯당 1건만 보고
