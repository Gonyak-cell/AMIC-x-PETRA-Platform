...
class TestTechSaaSModule:
    """TechSaaSModule 단위 테스트."""

    def test_registered_in_registry(self):
        """get_industry_module('tech') → TechSaaSModule 인스턴스."""
        module = get_industry_module("tech")
        assert isinstance(module, TechSaaSModule)

    def test_industry_id(self):
        """industry_id == 'tech'."""
        module = TechSaaSModule()
        assert module.industry_id == "tech"
        assert module.industry_name_kr == "테크/SaaS"
        assert module.industry_name_en == "Technology/SaaS"
    ...
    def test_kpis_required_ids(self):
        ...
        expected = {"arr", "nrr", "gross_churn", "ltv_cac", "rule_of_40"}
    ...
    def test_narrative_variant_type(self):
        """get_narrative_variant() → TechVariant."""
        module = TechSaaSModule()
        variant = module.get_narrative_variant()
        assert isinstance(variant, TechVariant)
