...
class TestColorClassifier:
    ...
    def test_achromatic_only_fallback(self) -> None:
        ...
        colors = [
            ...
            ExtractedColor(
                hex="#404040", rgb=(64, 64, 64), hsv=(0.0, 0.0, 0.251), ratio=0.3
            ),
            ExtractedColor(
                hex="#C0C0C0", rgb=(192, 192, 192), hsv=(0.0, 0.0, 0.753), ratio=0.2
            ),
        ]
    ...
    def test_hue_distance_requirement(self) -> None:
        ...
        colors = [
            ...
            ExtractedColor(
                hex="#FF3300", rgb=(255, 51, 0), hsv=(12.0, 1.0, 1.0), ratio=0.3
            ),
            ExtractedColor(
                hex="#0000FF", rgb=(0, 0, 255), hsv=(240.0, 1.0, 1.0), ratio=0.2
            ),
        ]
        classified = self.classifier.classify(colors)
        primary = [c for c in classified if c.role == "primary"]
    ...
    def test_single_chromatic_color(self) -> None:
        ...
        colors = [
            ExtractedColor(
                hex="#FF0000", rgb=(255, 0, 0), hsv=(0.0, 1.0, 1.0), ratio=1.0
            ),
        ]
        classified = self.classifier.classify(colors)
