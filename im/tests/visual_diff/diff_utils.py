...
> 마지막 수정: 2026-03-13 23:00:00

CI 전용 — LibreOffice headless + scikit-image 필요.
deal-mgmt/tests/visual_diff/diff_utils.py와 동일 인터페이스.
"""
...
def pptx_to_pngs(pptx_path: str, output_dir: str, dpi: int = 150) -> list[str]:
    """PPTX → PNG 변환 (LibreOffice headless → PDF → 페이지별 PNG).
...
def compute_ssim(
    ...
    if img_a.shape != img_b.shape:
        ...
        img_a = np.array(
            Image.fromarray(img_a).resize((target_w, target_h), Image.LANCZOS)
        )
        img_b = np.array(
            Image.fromarray(img_b).resize((target_w, target_h), Image.LANCZOS)
        )
