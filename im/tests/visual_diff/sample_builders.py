"""IM 테스트 데이터 빌더 — Single Source of Truth.

> 마지막 수정: 2026-03-14

generate_approved.py, test_visual_regression.py, conftest.py 가
동일 IMDocumentData 인스턴스를 공유하기 위한 순수 Python 모듈.
pytest 의존성 없음 — 스크립트에서도 직접 호출 가능.
"""
...
def build_titan_data() -> IMDocumentData:
...
def build_covenant_data() -> IMDocumentData:
...
def build_full_data() -> IMDocumentData:
...
def build_number_format_config() -> NumberFormatConfig:
    """기본 숫자 포맷 설정."""
    return NumberFormatConfig()


def build_usd_number_format_config() -> NumberFormatConfig:
    """USD 숫자 포맷 설정."""
    return NumberFormatConfig(
...
# variant 이름 → 빌더 함수 매핑
VARIANT_BUILDERS: dict[str, object] = {
    "tm_default": build_titan_data,
    "dm_default": build_covenant_data,
    "im_full": build_full_data,
}
