...
def _mock_transcript(audio_file_path: str) -> str:
    file_name = os.path.basename(audio_file_path)
    return (
        f"[Local STT fallback] Uploaded audio file: {file_name}.\n"
        "참석자들은 Project Uncore 진행 상황, 자료 보완, 후속 일정, 주요 리스크를 점검했습니다.\n"
        "자동 STT 외부 키가 없어 검증용 deterministic transcript를 생성했습니다."
    )


def _mock_minutes_json(transcript: str, job: TranscriptionJob) -> dict:
    return {
        "minutes": (
            f"# {job.title}\n\n"
            "- Local/dev fallback transcript was used because STT/LLM credentials are unavailable.\n"
            f"- Transcript preview: {transcript[:500]}"
        ),
        "summary": "Local/dev fallback generated deterministic meeting minutes for upload flow verification.",
        "key_issues": [
            {
                "title": "Local STT fallback",
                "description": "External STT or LLM credentials are not configured in this environment.",
                "priority": "MEDIUM",
            }
        ],
        "action_items": [],
        "condition_assessment": {
            "match_level": "NOT_ASSESSED",
            "notes": "Generated by local fallback.",
        },
        "buyer_reaction": "NEUTRAL",
    }
...
async def _run_stt(audio_file_path: str) -> str:
    ...
    if not clova_client_id or not clova_client_secret:
        if _local_fallback_enabled():
            logger.info("Using local STT fallback for %s", audio_file_path)
...
async def _run_llm_analysis(
    ...
    # 컨텍스트 구성
    if not llm.is_available and _local_fallback_enabled():
        logger.info("Using local LLM fallback for transcription job %s", job.id)
        return _mock_minutes_json(transcript, job), 0.0
