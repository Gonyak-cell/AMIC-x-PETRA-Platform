...
async def preview_financial_model_source_routing(
    db: AsyncSession,
    transaction_id: uuid.UUID,
    model_type: FinancialModelType,
) -> dict[str, Any]:
    target_workstreams = _get_financial_model_target_workstreams(model_type)
    source_files = await TextExtractionService().extract_from_vdr_documents(
        db,
        transaction_id,
    )
    if not source_files:
        return _build_financial_model_source_routing([], target_workstreams)
    ...
    return _build_financial_model_source_routing(routed_sources, target_workstreams)
