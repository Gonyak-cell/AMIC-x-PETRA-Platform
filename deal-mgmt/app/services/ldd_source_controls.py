...
SOURCE_CONTROL_HARD_BLOCK_CODES: frozenset[str] = frozenset(
    {
        ...
        "common_only_ldd_inputs",
        "common_document_overuse",
        "unresolved_evidence_refs",
        "missing_required_evidence",
        "missing_direct_evidence_for_material_issue",
        "routing_manual_review_block",
        "recommendation_summary_mismatch",
        "template_placeholder_remnants",
    }
)

_CORE_LDD_SECTION_TYPES: tuple[str, ...] = (
    "GOVERNANCE",
    "CAPITAL",
    "CONTRACTS",
)
_LAW_FIRM_CHAPTER_SECTIONS: dict[str, tuple[str, ...]] = {
    "I": ("GOVERNANCE", "CAPITAL"),
    "II": ("PERMITS", "TAX", "DATA_IT"),
    "III": ("CONTRACTS", "IP"),
    "IV": ("REAL_ESTATE",),
    "V": ("REAL_ESTATE",),
    "VI": ("LABOR",),
...
_COMMON_DOCUMENT_OVERUSE_RATIO = 0.60
_SUMMARY_RECOMMENDATION_ALIGNMENT_THRESHOLD = 60.0
...
def build_ldd_source_control_qa(
    ...
    *,
    ...
    routing_risks = _build_routing_input_risks(source_routing)
    core_coverage = _build_core_section_coverage(sections, evidence_ledger)
    summary_row_mismatches = _collect_summary_row_mismatches(qa_result, sections)
    ...
    if core_coverage["missing_core_sections"] or core_coverage["uncovered_core_sections"]:
        issues.append(
            _make_issue(
                "core_section_coverage_gap",
                "critical",
                "Core LDD sections are missing or still lack evidence-backed coverage.",
                details=[
                    {
                        "missing_core_sections": core_coverage["missing_core_sections"],
                        "uncovered_core_sections": core_coverage["uncovered_core_sections"],
                        "present_sections": core_coverage["present_sections"],
                        "covered_sections": core_coverage["covered_sections"],
                    }
                ],
                hard_block=True,
            )
    ...
    if routing_risks["manual_review_included"]:
        issues.append(
            _make_issue(
                "routing_manual_review_block",
                "critical",
                f"{len(routing_risks['manual_review_included'])} LDD input documents still require routing review.",
                details=routing_risks["manual_review_included"][:10],
                hard_block=True,
            )
        )
    elif manual_review_docs > 0:
        issues.append(
            _make_issue(
    ...
    if routing_risks["common_only"]:
        issues.append(
            _make_issue(
                "common_only_ldd_inputs",
                "critical",
                "LDD draft currently relies only on COMMON-routed documents without any LDD-specific source.",
                details=routing_risks["common_documents"][:10],
                hard_block=True,
            )
        )
    elif routing_risks["common_overuse"]:
        issues.append(
            _make_issue(
                "common_document_overuse",
                "major",
                "COMMON-routed documents dominate the LDD input set beyond the allowed ratio.",
                details=[
                    {
                        "common_document_ratio": routing_risks["common_ratio"],
                        "included_documents": routing_risks["included_documents"],
                        "common_documents": len(routing_risks["common_documents"]),
                        "ldd_specific_documents": len(routing_risks["ldd_documents"]),
                    },
                    *routing_risks["common_documents"][:10],
                ],
                hard_block=True,
            )
        )

    if summary_row_mismatches:
        issues.append(
            _make_issue(
                "recommendation_summary_mismatch",
                "critical",
                f"Executive summary recommendations diverge from detailed section recommendations in {len(summary_row_mismatches)} chapters.",
                details=summary_row_mismatches[:10],
                hard_block=True,
            )
        )
    ...
    return {
        ...
        "summary": {
            ...
            "manual_review_included_documents": len(routing_risks["manual_review_included"]),
            "common_documents_included": len(routing_risks["common_documents"]),
            "ldd_specific_documents_included": len(routing_risks["ldd_documents"]),
            "common_document_ratio": routing_risks["common_ratio"],
            "missing_core_sections": core_coverage["missing_core_sections"],
            "uncovered_core_sections": core_coverage["uncovered_core_sections"],
            "recommendation_summary_mismatches": len(summary_row_mismatches),
...
def _build_routing_input_risks(source_routing: dict[str, Any] | None) -> dict[str, Any]:
    ...
    common_documents = [
        _summarize_routing_document(doc)
        for doc in included_documents
        if COMMON_WORKSTREAM in set(doc.get("workstream_tags") or [])
        and LDD_WORKSTREAM not in set(doc.get("workstream_tags") or [])
    ]
    ldd_documents = [
        _summarize_routing_document(doc)
        for doc in included_documents
        if LDD_WORKSTREAM in set(doc.get("workstream_tags") or [])
        or str(doc.get("primary_workstream", "") or "") == LDD_WORKSTREAM
    ]
    common_ratio = round(len(common_documents) / len(included_documents), 3) if included_documents else 0.0
    return {
        "included_documents": len(included_documents),
        "manual_review_included": manual_review_included,
        "common_documents": common_documents,
        "ldd_documents": ldd_documents,
        "common_ratio": common_ratio,
        "common_only": bool(included_documents and common_documents and not ldd_documents),
        "common_overuse": bool(
            len(included_documents) >= 3
            and len(common_documents) > len(ldd_documents)
            and common_ratio > _COMMON_DOCUMENT_OVERUSE_RATIO
        ),
    }


def _summarize_routing_document(document: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "document_id": document.get("document_id"),
        "original_name": document.get("original_name"),
        "primary_workstream": document.get("primary_workstream"),
        "workstream_tags": list(document.get("workstream_tags") or []),
        "confidence": document.get("confidence"),
        "requires_manual_review": bool(document.get("requires_manual_review")),
    }


def _build_core_section_coverage(
    sections: list[dict] | None,
    evidence_ledger: dict[str, Any] | None,
) -> dict[str, list[str]]:
    present_sections = sorted(
        {
            ...
            for section in sections or []
            if str(section.get("section_type", "") or "")
        }
    )
    ...
    uncovered_core_sections = [
        ...
        if section_type in present_sections and section_type not in covered_sections
    ]
    return {
        "present_sections": present_sections,
        "covered_sections": covered_sections,
        "missing_core_sections": missing_core_sections,
        "uncovered_core_sections": uncovered_core_sections,
    }


def _collect_summary_row_mismatches(
    qa_result: dict[str, Any] | None,
    sections: list[dict] | None,
) -> list[dict[str, Any]]:
    summary_rows = list((qa_result or {}).get("summary_rows") or [])
    if not summary_rows or not sections:
        return []

    section_lookup = {
        str(section.get("section_type", "") or ""): list(section.get("items") or [])
        for section in sections or []
        if str(section.get("section_type", "") or "")
    }
    mismatches: list[dict[str, Any]] = []
    for row in summary_rows:
        mapped_sections = _resolve_summary_row_sections(row)
        if not mapped_sections:
            continue

        source_recommendations = [
            recommendation
            for section_type in mapped_sections
            for recommendation in _collect_issue_recommendations(section_lookup.get(section_type, []))
        ]
        if not source_recommendations:
            continue

        summary_recommendation = str(row.get("recommendation", "") or "").strip()
        if _summary_row_matches_recommendations(summary_recommendation, source_recommendations):
            continue
        ...
        mismatches.append(
            {
                ...
                "detail_recommendation_examples": source_recommendations[:3],
            }
        )
...
def _resolve_summary_row_sections(row: Mapping[str, Any]) -> tuple[str, ...]:
    chapter_number = str(row.get("chapter_number", "") or "").strip().upper()
    return _LAW_FIRM_CHAPTER_SECTIONS.get(chapter_number, ())


def _collect_issue_recommendations(items: list[dict[str, Any]]) -> list[str]:
    return [
        recommendation
        for item in items or []
        if str(item.get("status", "") or "") == "ISSUE"
        for recommendation in [str(item.get("recommendation", "") or "").strip()]
        if recommendation
    ]


def _summary_row_matches_recommendations(summary_recommendation: str, detail_recommendations: list[str]) -> bool:
    normalized_summary = _normalize_alignment_text(summary_recommendation)
    if not normalized_summary:
        return False

    for recommendation in detail_recommendations:
        normalized_detail = _normalize_alignment_text(recommendation)
        if not normalized_detail:
            continue
        if normalized_summary in normalized_detail or normalized_detail in normalized_summary:
            return True
        if fuzz.token_set_ratio(normalized_summary, normalized_detail) >= _SUMMARY_RECOMMENDATION_ALIGNMENT_THRESHOLD:
            return True
    return False
