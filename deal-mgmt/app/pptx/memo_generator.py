...
def add_logo(slide):
    ...
    if os.path.exists(logo):
        slide.shapes.add_picture(
            logo,
            Inches(LEFT_MARGIN), Inches(LOGO_TOP),
            Inches(LOGO_WIDTH), Inches(LOGO_HEIGHT)
        )
...
def add_textbox(slide, left, top, width, height, text, font_name=None,
    ...
    for para in tf.paragraphs:
        ...
        for run in para.runs:
            set_font(run, name=font_name, size_pt=font_size, bold=bold,
...
def add_info_table_block(slide, left, top, width, label, fields,
    ...
    if bullet_values is not None:
        ...
        lbl_shape = slide.shapes.add_textbox(
            Inches(left), Inches(top), Inches(lbl_w), Inches(block_h)
        )
        ...
        for i, line in enumerate(lines):
            ...
            run = p.add_run()
            run.text = line
            set_font(run, size_pt=10 if i == 0 else 8,
                     bold=(i == 0), color=COLOR_GREEN_ACCENT)

        val_left = left + lbl_w
        val_w = width - lbl_w
        txBox = slide.shapes.add_textbox(
            Inches(val_left), Inches(top), Inches(val_w), Inches(block_h)
        )
    ...
    lbl_shape = slide.shapes.add_textbox(
        Inches(left), Inches(top), Inches(label_w), Inches(total_h)
    )
    tf = lbl_shape.text_frame
    tf.word_wrap = True
    tf.vertical_anchor = MSO_ANCHOR.MIDDLE
    ...
    if fields:
        tbl_shape = slide.shapes.add_table(
            n_rows, 2,
            Inches(left + label_w), Inches(top),
...
def create_cover_slide(prs, project_name, memo_type_label, date_str):
    ...
    add_textbox(slide, LEFT_MARGIN, 2.8, 8.0, 0.8,
                project_name, font_size=36, bold=True,
                theme_color='BACKGROUND_1')

    add_textbox(slide, LEFT_MARGIN, 3.7, 8.0, 0.5,
                memo_type_label, font_size=20, bold=False,
                theme_color='BACKGROUND_1')

    if date_str:
        add_textbox(slide, LEFT_MARGIN, 4.3, 4.0, 0.4,
                    date_str, font_size=14,
                    theme_color='BACKGROUND_1')
...
def create_disclaimer_slide(prs, disclaimer_text, date_label='', company_name=''):
    ...
    body_box = slide.shapes.add_textbox(
        Inches(LEFT_MARGIN), Inches(1.191),
        Inches(4.7251), Inches(3.5758)
    )
...
def create_toc_slide(prs, sections, current_section=0):
    ...
    add_textbox(slide, LEFT_MARGIN, 1.497, 3.5517, 0.4166,
                "TABLE OF CONTENTS", font_size=24, bold=True, theme_color='BACKGROUND_1')
...
def create_section_divider_slide(prs, section_number, section_title):
    ...
    add_textbox(slide, LEFT_MARGIN, 3.0, 1.0, 0.5,
                str(section_number).zfill(2), font_size=48, bold=True,
                theme_color='BACKGROUND_1')

    add_textbox(slide, LEFT_MARGIN, 3.6, 5.0, 0.5,
                section_title, font_size=28, bold=True,
                theme_color='BACKGROUND_1')
...
def _place_body_element(slide, elem, current_top):
    ...
    if elem_type == 'text':
        ...
        height = lines * 0.25
        add_textbox(slide, LEFT_MARGIN, current_top, CONTENT_WIDTH, height,
                    content, font_size=font_size, bold=bold, color=COLOR_DARK,
                    alignment='left')
        ...
        add_section_header_shape(slide, LEFT_MARGIN, current_top, width, content,
                                 bg_color=bg_color)
        ...
        txBox = add_textbox(slide, LEFT_MARGIN + 0.2, current_top,
                            CONTENT_WIDTH - 0.2, height,
        ...
        if headers and rows:
            ...
            add_table(slide, LEFT_MARGIN, current_top,
                      CONTENT_WIDTH, height, headers, rows)
        ...
        height = lines * 0.25
        add_rich_text_box(slide, LEFT_MARGIN, current_top, CONTENT_WIDTH, height,
                          runs, base_font_size=font_size)
        return current_top + height + 0.15
        ...
        return add_info_table_block(slide, LEFT_MARGIN, current_top, CONTENT_WIDTH,
                                    label, fields, financial=financial,
...
def _place_body_element_at(slide, elem, left, top, width):
    ...
    if elem_type == 'text':
        ...
        txBox = add_textbox(slide, left + 0.1, top, width - 0.1, height,
                            '', font_size=font_size, color=COLOR_DARK)
        tf = txBox.text_frame
        tf.clear()
        for i, item in enumerate(items):
        ...
        height = lines * 0.22
        add_rich_text_box(slide, left, top, width, height,
                          runs, base_font_size=font_size)
        return top + height + 0.1
        ...
        financial = elem.get('financial', None)
        bullet_values = elem.get('bullet_values', None)
        return add_info_table_block(slide, left, top, width,
                                    label, fields, financial=financial,
                                    bullet_values=bullet_values)
...
def create_closing_slide(prs, contact_info=None):
    ...
    add_textbox(slide, LEFT_MARGIN, 3.0, 5.0, 0.5,
                "Thank You", font_size=36, bold=True,
                theme_color='BACKGROUND_1')

    if contact_info:
        add_textbox(slide, LEFT_MARGIN, 4.0, 5.0, 1.5,
                    contact_info, font_size=12,
                    theme_color='BACKGROUND_1')
...
def generate_from_json(prs, content):
    ...
    if disclaimer:
        create_disclaimer_slide(prs, disclaimer,
                                date_label=date_str,
    ...
    for slide_data in slides_data:
        ...
        if layout_name == 'SECTION_DIVIDER':
            create_section_divider_slide(
                prs,
                slide_data.get('section_number', 1),
            ...
            if slide_data.get('title'):
                add_textbox(slide, LEFT_MARGIN, 3.0, 5.0, 0.5,
                            slide_data['title'], font_size=28, bold=True,
                            theme_color='BACKGROUND_1')
            add_confidential_footer(slide, position='bottom', on_forest=True)
...
def _default_content(memo_type: str, project_code: str) -> dict:
    ...
    STRUCTURES: dict[str, dict] = {
        'tm': {
            'memo_type': 'Teaser Memo',
            'slides': [
                {'title': 'Investment Highlights', 'body': [
                    {'type': 'bullet', 'items': [
                        '[투자 포인트 1]', '[투자 포인트 2]', '[투자 포인트 3]',
                        '[투자 포인트 4]', '[투자 포인트 5]'
                    ]}
                ]},
                {'title': 'Company Overview', 'body': [
                    {'type': 'two_column', 'left': [
                        ...
                        {'type': 'table', 'headers': ['주주', '지분율'], 'rows': [
                            ['[주주1]', '[지분율]'], ['[주주2]', '[지분율]'], ['합계', '100%'],
                        ]}
                    ]}
                ]},
                {'title': 'Business Overview', 'body': [
                    {'type': 'text', 'content': '[사업 현황 요약]', 'bold': True, 'font_size': 14},
                    {'type': 'bullet', 'items': ['[사업 영역 1]', '[사업 영역 2]', '[사업 영역 3]']}
                ]},
                {'title': 'Financial Summary', 'body': [
                    {'type': 'table', 'headers': ['항목', '2023', '2024', '2025E'], 'rows': [
                        ['매출액', '[금액]', '[금액]', '[금액]'],
                        ['영업이익', '[금액]', '[금액]', '[금액]'],
                        ['EBITDA', '[금액]', '[금액]', '[금액]'],
                        ['순이익', '[금액]', '[금액]', '[금액]'],
                ...
                {'title': 'Transaction Overview', 'body': [
                    {'type': 'two_column', 'left': [
                        {'type': 'section_header', 'content': '거래 개요'},
                        {'type': 'table', 'headers': ['항목', '내용'], 'rows': [
                            ['거래 형태', '[거래 형태]'], ['대상 지분', '[대상 지분]'],
                            ['예상 거래규모', '[거래규모]'], ['희망 일정', '[일정]'],
                        ]}
                    ], 'right': [
        ...
        'dm': {
            ...
            'slides': [
                ...
                {'title': 'Discussion Background', 'body': [
                    {'type': 'text', 'content': '[논의 배경 상세 설명]'},
                    {'type': 'bullet', 'items': ['[배경 1]', '[배경 2]', '[배경 3]']}
                ]},
                {'title': 'Key Discussion Points', 'body': [
                    {'type': 'bullet', 'items': [
                        '[논의 포인트 1]', '[논의 포인트 2]',
                        '[논의 포인트 3]', '[논의 포인트 4]',
                    ]}
                ...
                {'title': 'Next Steps', 'body': [
                    {'type': 'table', 'headers': ['단계', '내용', '담당', '일정'], 'rows': [
                        ['1', '[액션 1]', '[담당자]', '[일정]'],
                        ['2', '[액션 2]', '[담당자]', '[일정]'],
                        ['3', '[액션 3]', '[담당자]', '[일정]'],
                    ]}
        ...
        'im': {
            ...
            'slides': [
                ...
                {'title': 'Investment Highlights (cont.)', 'body': [
                    {'type': 'text', 'content': '[투자 포인트 상세]'}
                ]},
                {'title': 'Company Overview', 'body': [
                    {'type': 'table', 'headers': ['항목', '내용'], 'rows': [
                        ['회사명', '[회사명]'], ['설립일', '[설립일]'],
                ...
                {'title': 'Business Overview', 'body': [
                    {'type': 'text', 'content': '[사업 모델 및 현황]'}
                ]},
                {'title': 'Market Analysis', 'body': [
                    {'type': 'text', 'content': '[시장 규모 및 성장성]'}
                ]},
                {'title': 'Competitive Landscape', 'body': [
                    {'type': 'text', 'content': '[경쟁 구도 분석]'}
                ]},
                {'title': 'Financial Summary (P&L)', 'body': [
                    {'type': 'table', 'headers': ['항목', '2022', '2023', '2024', '2025E', '2026E'], 'rows': [
                        ['매출액', '[금액]', '[금액]', '[금액]', '[금액]', '[금액]'],
                ...
                {'title': 'Financial Summary (B/S)', 'body': [
                    {'type': 'table', 'headers': ['항목', '2022', '2023', '2024'], 'rows': [
                        ['총자산', '[금액]', '[금액]', '[금액]'],
                        ['총부채', '[금액]', '[금액]', '[금액]'],
                        ['자기자본', '[금액]', '[금액]', '[금액]'],
                    ]}
                ]},
                {'title': 'Valuation Analysis', 'body': [
                    {'type': 'text', 'content': '[밸류에이션 분석 — EV/EBITDA, DCF 등]'}
                ]},
                {'title': 'Exit Strategy', 'body': [
                    {'type': 'bullet', 'items': ['[엑싯 시나리오 1]', '[엑싯 시나리오 2]', '[엑싯 시나리오 3]']}
                ]},
                {'title': 'Risk Factors', 'body': [
                    {'type': 'table', 'headers': ['구분', '내용', '대응방안'], 'rows': [
                        ['시장', '[리스크]', '[대응]'],
                        ['운영', '[리스크]', '[대응]'],
                        ['재무', '[리스크]', '[대응]'],
                    ]}
                ]},
                {'title': 'Investment Summary', 'body': [
                    {'type': 'bullet', 'items': [
                        '[핵심 투자 포인트 1]', '[핵심 투자 포인트 2]',
                        '[핵심 투자 포인트 3]', '[핵심 투자 포인트 4]'
                    ]}
                ]},
            ]
        },
        'proposal': {
            'memo_type': 'Proposal',
            'slides': [
                {'title': 'About AMIC × PETRA Bridge Partners', 'body': [
                    {'type': 'text', 'content': '[회사 소개]'}
                ]},
                {'title': 'Track Record', 'body': [
                    {'type': 'table', 'headers': ['연도', '프로젝트', '역할', '거래규모'], 'rows': [
                        ['[연도]', '[프로젝트]', '[역할]', '[규모]'],
                        ['[연도]', '[프로젝트]', '[역할]', '[규모]'],
                    ]}
                ]},
                {'title': 'Scope of Services', 'body': [
                    {'type': 'bullet', 'items': ['[서비스 범위 1]', '[서비스 범위 2]', '[서비스 범위 3]']}
                ]},
                {'title': 'Fee Structure', 'body': [
                    {'type': 'table', 'headers': ['구분', '금액', '비고'], 'rows': [
                        ['기본 보수', '[금액]', '[비고]'],
                        ['성공 보수', '[금액]', '[비고]'],
                    ]}
                ]},
