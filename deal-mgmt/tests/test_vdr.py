...
class TestVdrInit:
    async def test_init_vdr_creates_default_folders(
        self, client: AsyncClient, transaction_id: str
    ):
    ...
    async def test_init_vdr_duplicate_returns_400(
        self, client: AsyncClient, transaction_id: str
    ):
...
class TestVdrFolders:
    async def test_list_folders_returns_tree(
        self, client: AsyncClient, transaction_id: str
    ):
        await client.post(f"/api/v1/transactions/{transaction_id}/vdr/init")
    ...
    async def test_create_custom_folder(
        self, client: AsyncClient, transaction_id: str
    ):
        await client.post(f"/api/v1/transactions/{transaction_id}/vdr/init")
    ...
    async def test_delete_custom_folder(
        self, client: AsyncClient, transaction_id: str
    ):
        await client.post(f"/api/v1/transactions/{transaction_id}/vdr/init")
        create_resp = await client.post(
    ...
    async def test_delete_required_folder_returns_400(
        self, client: AsyncClient, transaction_id: str
    ):
        await client.post(f"/api/v1/transactions/{transaction_id}/vdr/init")
        folders_resp = await client.get(
        ...
        resp = await client.delete(
            f"/api/v1/transactions/{transaction_id}/vdr/folders/{required_folder['id']}"
        )
        assert resp.status_code == 400
...
class TestVdrDocuments:
    async def _init_and_get_folder(
        self, client: AsyncClient, transaction_id: str
    ) -> str:
    ...
    async def test_upload_invalid_mime_returns_400(
        ...
        resp = await client.post(
            f"/api/v1/transactions/{transaction_id}/vdr/folders/{folder_id}/documents",
            files={
                "file": ("test.exe", io.BytesIO(b"MZ"), "application/x-msdownload")
            },
        )
        assert resp.status_code == 400

    async def test_list_folder_documents(
        self, client: AsyncClient, transaction_id: str
    ):
        folder_id = await self._init_and_get_folder(client, transaction_id)
        # 파일 2개 업로드
        for name in ("doc1.pdf", "doc2.pdf"):
            await client.post(
                f"/api/v1/transactions/{transaction_id}/vdr/folders/{folder_id}/documents",
                files={
                    "file": (name, io.BytesIO(b"fake content"), "application/pdf")
                },
            )
    ...
    async def test_delete_document_soft_delete(
        ...
        upload_resp = await client.post(
            f"/api/v1/transactions/{transaction_id}/vdr/folders/{folder_id}/documents",
            files={
                "file": ("del.pdf", io.BytesIO(b"content"), "application/pdf")
            },
        )
    ...
    async def test_get_document_metadata(
        ...
        upload_resp = await client.post(
            f"/api/v1/transactions/{transaction_id}/vdr/folders/{folder_id}/documents",
            files={
                "file": ("meta.pdf", io.BytesIO(b"content"), "application/pdf")
            },
        )
        doc_id = upload_resp.json()["id"]
    ...
    async def test_update_document_description(
        self, client: AsyncClient, transaction_id: str
    ):
        folder_id = await self._init_and_get_folder(client, transaction_id)
        upload_resp = await client.post(
            f"/api/v1/transactions/{transaction_id}/vdr/folders/{folder_id}/documents",
            files={
                "file": ("upd.pdf", io.BytesIO(b"content"), "application/pdf")
            },
        )
        doc_id = upload_resp.json()["id"]
...
class TestVdrSummary:
    async def test_vdr_summary_not_initialized(
        self, client: AsyncClient, transaction_id: str
    ):
        resp = await client.get(
            f"/api/v1/transactions/{transaction_id}/vdr/summary"
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    async def test_vdr_summary_after_init_and_upload(
        self, client: AsyncClient, transaction_id: str
    ):
        await client.post(f"/api/v1/transactions/{transaction_id}/vdr/init")
        ...
        await client.post(
            f"/api/v1/transactions/{transaction_id}/vdr/folders/{folder_id}/documents",
            files={
                "file": ("sum.pdf", io.BytesIO(b"12345"), "application/pdf")
            },
        )
...
class TestVdrIsolation:
    async def test_cross_txn_folder_access_denied(
        self, client: AsyncClient, transaction_id: str
    ):
...
class TestVdrDocumentEdgeCases:
    ...
    async def test_download_deleted_document_returns_410(
        self, client: AsyncClient, transaction_id: str
    ):
        ...
        # 삭제된 문서 다운로드 시도
        resp = await client.get(
            f"/api/v1/transactions/{transaction_id}/vdr/documents/{doc_id}/download"
        )
    ...
    async def test_move_document_to_another_folder(
        self, client: AsyncClient, transaction_id: str
    ):
        """문서를 다른 폴더로 이동."""
        await client.post(f"/api/v1/transactions/{transaction_id}/vdr/init")
        ...
        list_b = await client.get(
            f"/api/v1/transactions/{transaction_id}/vdr/folders/{folder_b}/documents"
        )
        assert any(d["id"] == doc_id for d in list_b.json())

    async def test_upload_extension_mime_mismatch_returns_400(
        self, client: AsyncClient, transaction_id: str
    ):
    ...
    async def test_delete_folder_cascades_child_documents(
        self, client: AsyncClient, transaction_id: str
    ):
...
class TestVdrOverview:
    ...
    async def test_overview_includes_transaction(
        self, client: AsyncClient, transaction_id: str
    ):
    ...
    async def test_overview_shows_uninitialized(
        self, client: AsyncClient, transaction_id: str
    ):
    ...
    async def test_overview_after_init_and_upload(
        self, client: AsyncClient, transaction_id: str
    ):
        """VDR 초기화 + 업로드 후 통계 반영."""
        # VDR 초기화
