...
def _set_margins(doc: Document,
                 left: float = 2.54, right: float = 2.54,
...
def _add_title(doc: Document, text: str, size_pt: float = 16.0,
               space_before_pt: float = 60.0) -> None:
...
def _add_preamble_body(doc: Document, text: str,
                        indent_cm: float = 1.30) -> None:
...
def _add_signature_table(doc: Document, parties: list[tuple[str, str, str]]) -> None:
    ...
    note = doc.add_paragraph(
        "본 계약의 성립을 증명하기 위하여 아래에 서명 또는 기명날인한다."
    )
...
def create_spa_template(out_dir: Path) -> None:
    ...
    _add_preamble_text(doc,
        "{{ target_company_name }}(이하 \"대상회사\")는 대한민국 법령에 따라 설립되어 존속하는 주식회사이다."
    )
    _add_preamble_text(doc,
    ...
    _add_body(doc, "본 계약에서 사용되는 다음의 용어는 본 계약에서 달리 정의되지 아니하는 한, 아래 규정된 의미를 가진다.")
    _add_body(doc, "\"대상주식\"이라 함은 대상회사 발행의 보통주식 {{ transfer_shares }}주를 말한다.", indent_cm=1.30)
    _add_body(doc, "\"매매대금\"이라 함은 1주당 금 {{ share_price_per }}원으로 산정한 총 금 {{ total_purchase_price }}원을 말한다.", indent_cm=1.30)
    ...
    _add_body(doc, "\"진술보장 기간\"이라 함은 거래종결일로부터 {{ warranty_period_months }}개월을 말한다.", indent_cm=1.30)
    ...
    _add_body(doc, "본 계약에서 정하는 조건에 따라, 매도인은 거래종결일에 매수인에게 대상주식을 양도하고, 매수인은 이를 인수한다.")
    ...
    _add_body(doc, "본 계약에 따른 거래종결은 아래 각 호의 선행조건이 모두 충족(또는 권리자의 서면 포기)됨을 전제로 한다.")
...
def create_sha_template(out_dir: Path) -> None:
    ...
    _add_preamble_text(doc,
        "본 주주간계약(이하 \"본 계약\")은 {{ signing_date }}(이하 \"본 계약 체결일\") "
        "아래 당사자들 사이에서 체결되었다."
    )
    ...
    _add_section_label(doc, "전     문")
    _add_preamble_text(doc,
        "본 계약 체결일 현재 {{ company_name }}(이하 \"대상회사\")의 주주 구성은 다음과 같다."
    )
    _add_preamble_text(doc,
        "당사자들은 대상회사의 공동 운영과 당사자들의 권리·의무를 규율하기 위하여 "
        "본 계약을 체결한다."
    )
    _add_section_label(doc, "아     래")
...
def create_bta_template(out_dir: Path) -> None:
    ...
    _add_preamble_body(doc, "{{ transferee_name }}(이하 \"양수인\", 양도인과 양수인을 총칭하여 \"당사자들\")", indent_cm=1.50)
    ...
    _add_body(doc,
        "양도대상 영업의 대금은 금 {{ total_consideration }}원으로 한다."
    )
    _add_subheading(doc, "지급 방법.")
    _add_body(doc,
        "양수인은 거래종결일에 양도인이 서면으로 지정하는 계좌로 양도대금 전액을 지급한다."
    )
    ...
    _add_body(doc,
        "거래종결은 아래 각 호의 선행조건이 모두 충족(또는 포기)됨을 전제로 한다."
    )
    ...
    _add_body(doc,
        "본 계약은 {{ governing_law }} 법에 따라 해석되며, "
        "분쟁은 서울중앙지방법원을 제1심 전속 관할법원으로 한다."
    )
...
def create_ssa_template(out_dir: Path) -> None:
    ...
    _add_body(doc, "\"대상신주\"라 함은 발행회사가 제3자 배정 유상증자 방식으로 인수인에게 발행하는 {{ share_class }} {{ new_shares_count }}주를 말한다.", indent_cm=1.30)
    _add_body(doc, "\"신주인수대금\"이라 함은 대상신주의 1주당 발행금액 금 {{ subscription_price_per }}원에 대상신주의 수를 곱한 총 금 {{ total_investment }}원을 말한다.", indent_cm=1.30)
    ...
    _add_body(doc,
        "인수인은 대상회사의 이사회에 {{ board_seats }}인의 이사를 지명할 권리를 가진다."
    )
...
def create_mou_template(out_dir: Path) -> None:
    ...
    _add_preamble_body(doc, "{{ party_b_name }}(이하 \"매수인\", 매도인대표와 매수인을 총칭하여 \"당사자들\")", indent_cm=1.30)

    _add_section_label(doc, "전     문")
    _add_preamble_text(doc,
        "당사자들은 {{ purpose }}에 관한 기본적인 사항을 정하고자 본 양해각서를 체결한다."
    )
    ...
    _add_body(doc,
        "본 양해각서 중 다음 각 호의 조항은 당사자들을 법적으로 구속한다."
    )
    ...
    _add_body(doc,
        "본 양해각서는 {{ governing_law }} 법에 따라 해석되며, "
        "분쟁은 서울중앙지방법원을 제1심 전속 관할법원으로 한다."
    )
