...
permit_filing_type = sa.Enum(
    "CHANGE_NOTIFICATION",
    "CHANGE_APPROVAL",
    "NEW_REGISTRATION",
    "RENEWAL",
    name="permitfilingtype",
    create_type=False,
)
permit_timing_type = sa.Enum(
    "PRE_FILING",
    "POST_FILING",
    "BOTH",
    name="permittimingtype",
    create_type=False,
)
permit_analysis_status = sa.Enum(
    "PENDING",
    "ANALYZING",
    "COMPLETED",
    "FAILED",
    "MANUALLY_REVIEWED",
    name="permitanalysisstatus",
    create_type=False,
)
permit_requirement_status = sa.Enum(
    "IDENTIFIED",
    "DOCUMENTS_PREPARING",
    "FILED",
    "APPROVED",
    "NOT_APPLICABLE",
    name="permitrequirementstatus",
    create_type=False,
)
...
def upgrade() -> None:
    ...
    # Enum 타입 생성 (PostgreSQL에서만 명시적 생성 필요)
    if bind.dialect.name == "postgresql":
        permit_filing_type.create(bind, checkfirst=True)
        permit_timing_type.create(bind, checkfirst=True)
        permit_analysis_status.create(bind, checkfirst=True)
        permit_requirement_status.create(bind, checkfirst=True)
    ...
    op.create_table(
        ...
        sa.Column("business_types", _JSON, nullable=True),
        sa.Column("existing_permits", _JSON, nullable=True),
    ...
    op.create_table(
        ...
        sa.Column(
            "analysis_id",
            sa.Uuid(),
            sa.ForeignKey("permit_analyses.id", ondelete="CASCADE"),
            nullable=False,
        ),
        ...
        sa.Column("required_documents", _JSON, nullable=True),
        ...
        sa.Column(
            "compliance_item_id",
            sa.Uuid(),
            sa.ForeignKey("compliance_items.id", ondelete="SET NULL"),
            nullable=True,
        ),
    ...
    op.add_column(
        ...
        sa.Column(
            ...
            sa.Uuid(),
            sa.ForeignKey("permit_requirements.id", ondelete="SET NULL"),
            nullable=True,
        ),
    )
...
def downgrade() -> None:
    ...
    bind = op.get_bind()
    if bind.dialect.name == "postgresql":
        permit_requirement_status.drop(bind, checkfirst=True)
        permit_analysis_status.drop(bind, checkfirst=True)
        permit_timing_type.drop(bind, checkfirst=True)
        permit_filing_type.drop(bind, checkfirst=True)
