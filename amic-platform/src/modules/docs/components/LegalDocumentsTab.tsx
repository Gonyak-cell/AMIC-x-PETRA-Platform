...
import { Plus, Download, Trash2, RefreshCw, FileText, AlertCircle } from "lucide-react";
import { useLegalDocuments, useDeleteLegalDocument, useRegenerateLegalDocument, getLegalDocDownloadUrl, formatFileSize } from "@/modules/docs/hooks/useLegalDocuments";
import type { LegalDocument, LegalDocType } from "@/modules/docs/types/legal_document";
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-text-secondary">SPA, SHA, BTA, SSA 생성 및 다운로드</p>
        </div>
        <button
          type="button"
          onClick={() => handleNew()}
      ...
      {!docs || docs.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-border py-12 text-center">
          ...
          <div>
            <p className="text-sm font-medium text-text-secondary">생성된 법률 문서가 없습니다</p>
            <p className="text-xs text-text-tertiary">위 버튼으로 SPA, SHA 등 법률 문서를 생성하세요</p>
          </div>
        </div>
      ) : (
        <div className="overflow-hidden rounded-xl border border-border">
          <table className="w-full text-sm">
            <thead className="bg-white">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-text-tertiary">유형</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-text-tertiary">제목</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-text-tertiary">상태</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-text-tertiary">크기</th>
              ...
              {docs.map((doc: LegalDocument) => (
                <tr key={doc.id} className="hover:bg-white-elevated/50">
                  <td className="px-4 py-3">
                    <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${DOC_TYPE_BADGE[doc.doc_type]}`}>
                      {doc.doc_type}
                    </span>
                  </td>
                  <td className="max-w-[200px] px-4 py-3">
                    <p className="truncate text-sm font-medium text-text-primary">{doc.title}</p>
                    <p className="text-xs text-text-tertiary">{LEGAL_DOC_META[doc.doc_type].labelKo}</p>
                  </td>
                  <td className="px-4 py-3">
                    ...
                    {doc.status === "FAILED" && doc.error_message && (
                      <div className="mt-1 flex items-center gap-1 text-xs text-negative">
                        ...
                        <span className="truncate max-w-[120px]" title={doc.error_message}>오류 발생</span>
                      </div>
                    )}
                  </td>
                    ...
                    <div className="flex items-center justify-end gap-1">
                      {doc.status === "READY" && (
                        <a
                          ...
                          download={doc.file_name ?? `${doc.doc_type}_${doc.id}.docx`}
                      ...
                      <button
                        ...
                        title={confirmDeleteId === doc.id ? "다시 클릭하면 삭제됩니다" : "삭제"}
