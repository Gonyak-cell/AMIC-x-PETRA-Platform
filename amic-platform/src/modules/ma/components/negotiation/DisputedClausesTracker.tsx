        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-1 p-2">
            {filtered.map((issue) => {
              ...
              const hasLinked =
                issue.linked_issue_ids && issue.linked_issue_ids.length > 0;
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        <h4 className="truncate text-xs font-semibold text-text-dark">
                          {issue.title}
                        </h4>
                        {hasLinked && (
                    ...
                    <div className="flex gap-1 border-t border-gray-100 px-3 py-1.5">
                      {(
                        [
                          ...
                          "CONSIDER_ACCEPTING",
                          "CANNOT_ACCEPT",
                        ] as IssueDecisionStatus[]
                      ).map((ds) => {
                        const isActive = issue.decision_status === ds;
                        const label =
                          ISSUE_DECISION_STATUS_OPTIONS.find(
                            (o) => o.value === ds,
                          )?.label ?? ds;
                        return (
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              onUpdateDecision(issue.id, ds);
                            }}
                            className={`flex-1 rounded px-2 py-1 text-[10px] font-medium transition-colors ${
                              isActive
                                ? DECISION_ACTIVE_STYLE[ds]
                                : DECISION_STYLE[ds]
                            }`}
                          >
                            {label}
                    ...
                    <div className="border-t border-gray-100 px-3 py-3">
                      ...
                      {issue.ai_suggestion && (
                        <div className="mt-2 rounded bg-green-50 p-2">
                          ...
                          <p className="mt-0.5 text-xs text-green-900 whitespace-pre-wrap">
                            {issue.ai_suggestion}
                          </p>
                          {issue.ai_suggestion_rationale && (
                            <p className="mt-1 text-[10px] text-green-700 italic">
                              {issue.ai_suggestion_rationale}
                            </p>
                          )}
                        </div>
