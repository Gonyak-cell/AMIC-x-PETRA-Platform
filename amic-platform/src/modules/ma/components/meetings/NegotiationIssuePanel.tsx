  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {canWrite && (
          <Button icon={Plus} onClick={() => setShowForm(true)}>이견 등록</Button>
        )}
      </div>
        ...
        {issues.map((issue) => (
          <Card key={issue.id} className="p-4">
            <div className="flex items-start justify-between mb-2">
              <div>
                <h4 className="text-sm font-semibold text-text-dark">{issue.title}</h4>
                {issue.clause_reference && (
                  <span className="text-xs text-text-muted">{issue.clause_reference}</span>
                )}
              </div>
              <div className="flex items-center gap-2">
                ...
                <Badge
                  variant={issue.priority === "CRITICAL" ? "error" : issue.priority === "HIGH" ? "warning" : "neutral"}
                                  >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3">
              <div className="rounded-lg bg-blue-50 border border-blue-200 p-3">
                <p className="text-xs font-semibold text-blue-700 mb-1">우리측 입장</p>
                <p className="text-xs text-blue-900 whitespace-pre-wrap">
                  {issue.our_position || "-"}
                </p>
              </div>
              <div className="rounded-lg bg-orange-50 border border-orange-200 p-3">
                <p className="text-xs font-semibold text-orange-700 mb-1">상대측 입장</p>
                <p className="text-xs text-orange-900 whitespace-pre-wrap">
                  {issue.counterpart_position || "-"}
                </p>
              </div>
              <div className="rounded-lg bg-purple-50 border border-purple-200 p-3">
                <p className="text-xs font-semibold text-purple-700 mb-1">법률 검토</p>
            ...
            {issue.ai_suggestion && (
              <div className="mt-3 rounded-lg bg-green-50 border border-green-200 p-3">
                ...
                <p className="text-xs text-green-900 whitespace-pre-wrap">{issue.ai_suggestion}</p>
              </div>
            )}
            ...
            {issue.resolution && (
              <div className="mt-2 rounded-lg bg-gray-50 border border-gray-200 p-2">
                <p className="text-xs text-text-secondary">해결: {issue.resolution}</p>
              </div>
            )}
      ...
      {/* 생성 모달 */}
      <Modal open={showForm} onClose={resetForm} title="협상 이견 등록" size="lg">
        <div className="space-y-4">
          <Input label="제목" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="이견 제목" required />
