...
import { useRFI, useSendRFI, useCloseRFI, useExportRFI, useSyncRFIToChecklists, useExtendRFIDeadline } from "@/modules/ma/hooks/useRFI";
import RFIItemRow from "./RFIItemRow";
import type { RFICategory, RFIItemStatus, RFIItemPriority, RFIStatus } from "@/modules/ma/types/rfi";
import { RFI_STATUS_LABELS, RFI_CATEGORY_LABELS } from "@/modules/ma/constants";
...
export default function RFIDetailView({ txnId, rfiId, onBack, onRespond, onReview }: RFIDetailViewProps) {
  ...
  const [categoryFilter, setCategoryFilter] = useState<RFICategory | "ALL">("ALL");
  const [statusFilter, setStatusFilter] = useState<RFIItemStatus | "ALL">("ALL");
  const [priorityFilter, setPriorityFilter] = useState<RFIItemPriority | "ALL">("ALL");
  ...
  const groupedItems = useMemo(() => {
    if (categoryFilter !== "ALL") return [{ category: categoryFilter, items: filteredItems }];
  ...
  if (isError) {
    return (
      <div className="text-center py-12">
        <p className="text-red-500 font-medium">RFI를 불러오는 중 오류가 발생했습니다.</p>
        <p className="text-xs text-gray-500 mt-1">잠시 후 다시 시도해 주세요.</p>
        <Button variant="ghost" icon={ArrowLeft} onClick={onBack} size="sm" className="mt-4">
          목록으로
        </Button>
      </div>
    );
  }
  if (!rfi) return <div className="text-center py-12 text-gray-500">RFI를 찾을 수 없습니다</div>;

  const responsePct = rfi.total_items > 0 ? Math.round((rfi.responded_items / rfi.total_items) * 100) : 0;
  const acceptedPct = rfi.total_items > 0 ? Math.round((rfi.accepted_items / rfi.total_items) * 100) : 0;
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h3 className="text-lg font-bold text-gray-900 truncate">{rfi.title}</h3>
        ...
        <div className="flex gap-2">
          ...
          {rfi.status !== "CLOSED" && rfi.status !== "CANCELLED" && rfi.status !== "DRAFT" && (
            <>
              <Button
                size="sm"
                variant="outline"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <KpiCard
          label="응답률"
          value={`${responsePct}%`}
          variant={responsePct >= 80 ? "positive" : responsePct >= 50 ? "default" : "negative"}
        />
        <KpiCard
          label="확인 완료"
        ...
        <KpiCard
          ...
          variant={rfi.total_items - rfi.responded_items > 0 ? "negative" : "positive"}
        />
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex items-center gap-2" role="radiogroup" aria-label="RFI 카테고리 필터">
          <span className="text-xs font-medium text-gray-500">분류:</span>
          {[{ value: "ALL", label: "전체" }, ...categories.map((c) => ({ value: c, label: RFI_CATEGORY_LABELS[c] || c }))].map((opt) => (
            <button
              key={opt.value}
              type="button"
              role="radio"
              ...
              onClick={() => setCategoryFilter(opt.value as RFICategory | "ALL")}
            >
              {opt.label}
            </button>
          ))}
        ...
        <div className="flex items-center gap-2 ml-4" role="radiogroup" aria-label="RFI 항목 상태 필터">
          ...
          {([
            ...
            <button
              ...
              onClick={() => setStatusFilter(opt.value as RFIItemStatus | "ALL")}
            >
              {opt.label}
            </button>
          ))}
        ...
        <div className="flex items-center gap-2 ml-4" role="radiogroup" aria-label="RFI 우선순위 필터">
          ...
          {([
            ...
            <button
              ...
              onClick={() => setPriorityFilter(opt.value as RFIItemPriority | "ALL")}
            >
              {opt.label}
            </button>
          ))}
