...
const REACTION_VARIANT: Record<string, "success" | "warning" | "error" | "neutral"> = {
...
const CONDITION_VARIANT: Record<string, "success" | "warning" | "error" | "neutral"> = {
  FULL_MATCH: "success",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <KpiCard label="활성 매수자" value={String(data.buyer_summaries.length)} />
...
function MaterialRow({ material: m }: { material: MaterialDistributionForClient }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3">
      ...
      {m.distributed_at && (
        <span className="text-xs text-text-muted">{m.distributed_at.slice(0, 10)}</span>
      )}
    </div>
  );
}
  ...
  return (
    <div className="rounded-lg border border-gray-border p-4 space-y-2">
      <div className="flex items-center justify-between">
        ...
        <Badge variant="neutral" className="text-xs">{b.status_label}</Badge>
      </div>

      <div className="flex flex-wrap gap-2">
        {b.latest_reaction && (
          <Badge variant={REACTION_VARIANT[b.latest_reaction] ?? "neutral"} className="text-xs">
            {REACTION_LABEL[b.latest_reaction] ?? b.latest_reaction}
          </Badge>
        )}
        {b.condition_match && (
          <Badge variant={CONDITION_VARIANT[b.condition_match] ?? "neutral"} className="text-xs">
      ...
      {b.condition_notes && (
        <p className="text-xs text-text-muted line-clamp-2">{b.condition_notes}</p>
      )}
...
function MeetingRow({ meeting: m }: { meeting: UpcomingMeetingForClient }) {
  return (
    <div className="flex items-center gap-3 px-4 py-3">
      ...
      <Badge variant="neutral" className="text-xs">{m.channel}</Badge>
      {m.attendee_count > 0 && (
